...
const CanvasControls: React.FC<CanvasControlsProps> = ({ canvasId }) => {
    ...
    return (
        <div className="absolute bottom-4 right-4 flex flex-col gap-2 bg-surface rounded-lg p-2 shadow-lg z-10">
            <IconButton icon="+" title="Add Node" onClick={handleAddNode} />
            <IconButton
                icon="↑"
                title="Go Up"
                onClick={handleGoUp}
            ...
            />
            <IconButton icon="🔍+" title="Zoom In" onClick={handleZoomIn} />
            <IconButton icon="🔍-" title="Zoom Out" onClick={handleZoomOut} />
            <IconButton icon="⟳" title="Reset View" onClick={handleResetView} />
        </div>
    )
}
