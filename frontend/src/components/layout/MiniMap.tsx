...
const MiniMap: React.FC<MiniMapProps> = ({ canvasId, width = 200, height = 120 }) => {
    ...
    return (
        <div className="w-full h-full bg-background rounded relative">
            <svg width={width} height={height} className="w-full h-full" onClick={handleClick}>
                {/* Draw edges */}
                {canvas.edges.map((edgeRef) => {
                    ...
                    const sourceX =
                        (sourceNode.position.x + sourceNode.size.width / 2 - bounds.minX) * scale
                    const sourceY =
                        (sourceNode.position.y + sourceNode.size.height / 2 - bounds.minY) * scale
                    const targetX =
                        (targetNode.position.x + targetNode.size.width / 2 - bounds.minX) * scale
                    const targetY =
                        (targetNode.position.y + targetNode.size.height / 2 - bounds.minY) * scale
